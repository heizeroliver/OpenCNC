...
The service saves configuration, existing-folder enrollment baseline, attempts, and job history through `SqliteAgentStore`. Discovery/conversion remains in `packages/agent-core/src/node-workspace.ts`, and conversion remains in `packages/converter`. Project discovery is recursive and managed output directories are excluded. The service rechecks the names and SHA-256 values of verified BPP outputs, so missing output re-enrolls an otherwise unchanged CIX project while modified output still fails closed as a conflict. Case/Unicode-equivalent Windows collisions and read-during-export changes fail before a production write. Electron-builder packages an x64 assisted NSIS installer; Windows GitHub Actions is the authoritative native build/test environment.
