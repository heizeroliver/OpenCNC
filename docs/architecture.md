...
Both adapters require an export fingerprint to remain unchanged across two watcher scans before automatic conversion. The Windows agent recursively maps the configured tree, enrolls only directories created after its stored baseline, and writes each project to its resolved output pattern (`{projectName}_bpp` by default). `opencnc-sync-manifest.json` records source size, modification time, source checksum, output checksum, and all four verification claims. An existing BPP is updated only when its current checksum equals the previous manifest checksum. Missing verified output re-enrolls the project; unknown or manually edited outputs block the entire project write. Removed-source outputs are reported as orphans and preserved rather than deleted.
