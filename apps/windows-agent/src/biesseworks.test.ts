...
describe("BiesseWorks verified batch launcher", () => {
  it("opens every checksum-matched output from one completed conversion", async () => {
    ...
    const result = await openVerifiedBppOutputs(job(root, Object.keys(contents), contents), async path => { opened.push(path); return ""; }, { platform: "win32" });
  ...
  it("rejects unsafe or unverified history without opening anything", async () => {
    ...
    const opened: string[] = [];
    await expect(openVerifiedBppOutputs({ ...base, verified: false }, async path => { opened.push(path); return ""; }, { platform: "win32" })).rejects.toThrow("fully verified");
  ...
  it("checks the whole batch before launch and blocks a changed output", async () => {
    ...
    await expect(openVerifiedBppOutputs(job(root, Object.keys(contents), contents), async path => { opened.push(path); return ""; }, { platform: "win32" })).rejects.toThrow("changed after its verified conversion");
    expect(opened).toEqual([]);
  });

  it("reports a missing Windows file association after attempting the complete batch", async () => {
